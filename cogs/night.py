...
async def start_night(self: 'GameCog', channel: discord.TextChannel) -> None:
    ...
    # 人狼と霊界には通知ありでメッセージを送る
    if game.wolf_channel:
        await game.wolf_channel.send(f"🌙 **第{game.day_count}夜**：作戦会議の時間です。襲撃先を相談してください。", silent=False)
    if game.dead_channel:
        await game.dead_channel.send(f"🌙 **第{game.day_count}夜**：生存者たちの行動を見守りましょう。", silent=False)
    ...
    for player, role in game.roles.items():
        ...
        if role.name == RoleName.MEDIUM:
            if game.last_executed:
                ...
                try:
                    m_msg = await player.send(f"👻 【霊媒結果】: 昨日追放された {game.last_executed.display_name} は 【**{result_team}**】 でした。", silent=True)
                    game.add_dm_message(player.id, m_msg.id)
                except: pass
            else:
                try:
                    await player.send("👻 【霊媒結果】: 昨日追放された人はいませんでした。", silent=True)
                except: pass
        ...
        if label:
            ...
            try:
                ...
                msg = await player.send(f"【{role.name}】の能力発動時刻です。今夜の「{label}」対象者を選んでください。", view=view, silent=not is_wolf)
            ...
            try:
                i_msg = await player.send("🎒 **【夜間の身支度】** 明日の過酷な議論に備え、手荷物を確認しましょう。下のボタンからアイテムを1つ獲得できます。", view=ItemDrawView(channel.guild.id, timeout=game.night_time), silent=True)
...
async def process_night_results(self: 'GameCog', channel: discord.TextChannel) -> None:
        ...
        for actor, data in game.actions.items():
            if data['action'] == "強奪" and actor in game.alive_players:
                ...
                if target in game.alive_players:
                    ...
                    try:
                        t_msg = await actor.send(f"🎭 【怪盗の強奪結果】: {target.display_name} から役職を奪いました！あなたの新しい役職は 【**{target_role.name}**】 です。", silent=True)
    ...
    for actor, data in game.actions.items():
        ...
        if action == "占い":
            ...
            try:
                s_msg = await actor.send(f"🔮 【占い結果】: {target.display_name} を占いました。結果は 【**{result_team}**】 です。", silent=True)
            ...
            try:
                c_msg = await actor.send(f"🌀 【混乱成功】: {target.display_name} を混乱させました。", silent=True)
    ...
    if dead_list:
        ...
        # 霊界には先に通知を送って準備させる
        if game.dead_channel:
            await game.dead_channel.send("☀️ 朝が来ました。犠牲者を確認してください。", silent=False)
        ...
        await target_channel.send(embed=embed, silent=True)
        if game.dead_channel:
            await game.dead_channel.send("☀️ 朝が来ましたが、昨夜は犠牲者がいなかったようです。", silent=False)
